...
public partial class Pedidos_CadastroPedidos : PageBase
{
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        if (!IsPostBack)
            try
            {
                ...
                //Retirar a parte de OS para quem não possui acesso à tela de OS
                mtvOS.ActiveViewIndex = telas.Contains(new Tela(8)) ? 1 : 0;
                ...
                else if (Request["o"] != null)
                {
                    ...
                    if (this.OrcamentoCadastrar != null)
                    {
                        ...
                        {
                            if (this.OrcamentoCadastrar.DataCancelamento.CompareTo(SqlDate.MinValue) < 0)
                                this.OrcamentoCadastrar.DataCancelamento = SqlDate.MinValue;
                            if (this.OrcamentoCadastrar.DataRecusa.CompareTo(SqlDate.MinValue) < 0)
                                this.OrcamentoCadastrar.DataRecusa = SqlDate.MinValue;
                            if (this.OrcamentoCadastrar.DataEnvio.CompareTo(SqlDate.MinValue) < 0)
                                this.OrcamentoCadastrar.DataEnvio = SqlDate.MinValue;
                            lblOrcamentoVinculado.Text = this.OrcamentoCadastrar.Numero;
                            chkContratoFixo.Checked = (this.OrcamentoCadastrar.Itens != null && this.OrcamentoCadastrar.Itens.Count(item => item.Periodico) > 0);
    ...
    private void CarregarClientes()
    {
        ...
        ddlClientePedido.Items.Add(new ListItem("-- Selecione --", "0"));

        IList<Cliente> clientes = Cliente.ConsultarTodosOrdemAlfabetica();
        foreach (Cliente cli in clientes)
            ddlClientePedido.Items.Add(new ListItem(cli.NomeRazaoSocial + " - " + cli.CpfCnpj, cli.Id.ToString()));
    }
    ...
    private void CarregarFinanceiro(Pedido pedido)
    {
        ...
        if (pedido != null)
        {
            ...
            else
            {
                ...
                chkXMarkEdicao.Checked = pedido.Receita.XMark;
    ...
    private void CarregarOS(int idOS)
    {
        ...
        if (ordem != null)
        {
            ...
            //Pode editar prazo legal se nenhum prazo foi definido ainda
            tbxPrazoLegalOS.Enabled = btnCalcularPrazoLegal.Enabled = string.IsNullOrEmpty(tbxPrazoLegalOS.Text.Trim());

            tbxPrazoDiretoriaOS.Text = ordem.PrazoDiretoria != SqlDate.MinValue ? ordem.PrazoDiretoria.ToShortDateString() : "";
            //Pode editar prazo diretoria se nenhum prazo foi definido ainda
            tbxPrazoDiretoriaOS.Enabled = btnCalcularPrazoDiretoria.Enabled = string.IsNullOrEmpty(tbxPrazoDiretoriaOS.Text.Trim());
    ...
    private void SalvarPedido()
    {
        ...
        {
            pedido = new Pedido();
            pedido.DataAceiteContrato = SqlDate.MinValue;
            pedido.DataEnvioContrato = SqlDate.MinValue;
        }
        ...
        //Se detalhamento está em modo de edição, salvar um novo detalhamento para o pedido
        ...
        {
            if (pedido.OrdensServico == null)
                pedido.OrdensServico = new List<OrdemServico>();
            this.OrcamentoCadastrar = this.OrcamentoCadastrar.ConsultarPorId();
            //Se a forma de pagamento selecionada tem desconto ou acréscimo, adicionar nas OSs também
            decimal percentualAcrescimoDesconto = this.OrcamentoCadastrar.FormaDePagamentoSelecionada != null ?
                this.OrcamentoCadastrar.FormaDePagamentoSelecionada.AcrescimoDesconto : 0;

            foreach (ItemOrcamento item in this.OrcamentoCadastrar.Itens)
            {
                for (int index = 0; index < item.Quantidade; index++)
                {
                    OrdemServico os = new OrdemServico();
                    os.Renovavel = item.Periodico;
                    //Se for periódico tem que aumentar a data da OS
                    os.Data = (item.Periodico ? DateTime.Now.AddMonths(index) : DateTime.Now);
                    ...
                    //Aplicar o desconto ou o acréscimo da forma de pagamento em cada OS
                    if (percentualAcrescimoDesconto != 0)
                    {
                        decimal valorAcrescimoDesconto = (item.ValorTotal * percentualAcrescimoDesconto) / 100;
                        //Tem que negar o valor porque o desconto na forma de pagamento é negativa, mas no pedido é positiva
                        os.Desconto += -(valorAcrescimoDesconto);
                    }
        ...
        if (pedido.Receita != null)
        {
            pedido.Receita.XMark = chkXMarkEdicao.Checked;
    ...
    private void EnviarEmailAberturaPedido(Pedido pedido)
    {
        ...
        mail.EnviarAutenticado("Pedido realizado", this.FuncionarioLogado, pedido, 587, false);
    }
    ...
    private void SalvarOS()
    {
        ...
        //Enviar e-mail se é nova OS ou se ela ainda não tinha responsável
        bool enviarEmail = (ordem.Responsavel == null);

        if (tbxPrazoLegalOS.Enabled && ordem.PrazoLegal.CompareTo(SqlDate.MinValue) < 1)
            ordem.PrazoLegal = tbxPrazoLegalOS.Text.ToDateTime();
        if (tbxPrazoDiretoriaOS.Enabled && ordem.PrazoDiretoria.CompareTo(SqlDate.MinValue) < 1)
        ...
        String mensagemAtual = msg.Mensagem;
        msg.CriarMensagem("Ordem de serviço salva com sucesso. " + mensagemAtual, "Sucesso", MsgIcons.Sucesso);
    }
    ...
    private void ReplicarOSPeriodica(OrdemServico ordem)
    {
        foreach (DateTime dataReplicar in this.DatasParaReplicacao)
        {
            ...
            ordemNova.PrazoPadrao = (ordem.TipoRenovacao == OrdemServico.MESES ? ordemNova.Data.AddMonths(tipo.PrazoPadrao) : ordemNova.Data.AddDays(tipo.PrazoPadrao));
    ...
    {
        LogEventosOS log = LogEventosOS.SalvarLog(this.FuncionarioLogado, "Envio OS para o responsável", ordem.Codigo, "Início do processo");
        Email mail = new Email();
        ...
        log.adicionarLog("Carregando os dados do conteúdo do e-mail");
    ...
    <div>
        ...
        {
            log.adicionarLog("Adicionando responsável da OS");
        ...
        log.adicionarLog("Adicionando destinatários do email");
        ...
        {
            if (mail.EnviarAutenticado("Cadastro de OS", this.FuncionarioLogado, ordem.Pedido, 587, false))
            {
                log.adicionarLog("Email enviado!");
                msg.CriarMensagem("E-mail de cadastro de OS enviado com suscesso!", "Sucesso", MsgIcons.Sucesso);
            }
            else
            {
                log.adicionarLog("Email NÃO enviado. " + mail.Erro);
                msg.CriarMensagem("E-mail de cadastro de OS NÃO foi enviado!", "Erro", MsgIcons.Erro);
            }
        }
        else
        {
            log.adicionarLog("Sem destinatários para envio do e-mail!");
        }
    }
    ...
    private void GerarMovimentacaoPedido(Pedido pedido)
    {
        ...
        pedido = pedido.ConsultarPorId();
        if (pedido == null)
        {
            msg.CriarMensagem("Houve um problema ao carregar o pedido! Recarregue a página e tente novamente.", "Erro", MsgIcons.Alerta);
            return;
        }
        if (pedido.Receita != null)
        {
        ...
        receita.XMark = chkXMark.Checked;
        ...
        //Vamo ver se vai dar problema agora
        Transacao.Instance.Recarregar();
        pedido = pedido.ConsultarPorId();
    ...
    private void AdicionarParcelaAoPedido()
    {
        ...
        novaParcela.DataPagamento = SqlDate.MinValue;
    ...
    private void CarregarEdicaoParcela(Parcela parcela)
    {
        ...
        if (!parcela.IsPodeEditar)
        {
            msg.CriarMensagem("Não é possível alterar uma parcela " + parcela.GetDescricaoStatus, "Info", MsgIcons.Alerta);
            return;
        }
    ...
    protected void gdvOrdensServico_RowDeleting(object sender, GridViewDeleteEventArgs e)
    {
        try
        {
            ...
            Transacao.Instance.Recarregar();
            Pedido pedidoAux = Pedido.ConsultarPorId(hfId.Value.ToInt32());
            this.CarregarGridOrdensServico(pedidoAux);
            this.CarregarFinanceiro(pedidoAux);
        }
        catch (Exception ex)
        {
            msg.CriarMensagem(ex);
    ...
    {
        WebUtil.AdicionarConfirmacao((Button)sender, "Deseja realmente excluir este arquivo ?");
    }
